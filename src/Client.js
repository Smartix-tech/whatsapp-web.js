...
class Client extends EventEmitter {
  ...
  async initialize () {
    ...
    await page.evaluate(
      async function (selectors) {
        const observer = new MutationObserver(function () {
          const progressBar = window.getElementByXpath(
            selectors.PROGRESS
          )
          const progressMessage = window.getElementByXpath(
    ...
    if (needAuthentication) {
      ...
      const QRCONTAINER = 'div[data-ref]'
      const QR_RETRYBUTTON = 'div[data-ref] > span > button'
      let qrRetries = 0
      ...
      await page.evaluate(function (selectors) {
        const qrContainer = document.querySelector(selectors.QRCONTAINER)
        ...
        const obs = new MutationObserver((muts) => {
          muts.forEach(mut => {
            ...
            if (mut.type === 'childList') {
              const retryButton = document.querySelector(selectors.QR_RETRYBUTTON)
              if (retryButton) retryButton.click()
            }
          })
        })
        obs.observe(qrContainer.parentElement, {
          subtree: true,
          childList: true,
          attributes: true,
    ...
    await page.exposeFunction('onChangeMessageTypeEvent', (msg) => {
      if (msg.type === 'revoked') {
        ...
        /**
                 ...
                 * @param {?Message} revokedMsg The message that was revoked, before it was revoked. It will contain the message's original data.
